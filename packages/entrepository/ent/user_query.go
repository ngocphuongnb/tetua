...
// the "avatar_image" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAvatarImage; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "avatar_image_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.AvatarImage = n
			}
		}
	}
