	...
	if query := pq.withTopics; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(post.TopicsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "topics": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "topics" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Topics = append(nodes[i].Edges.Topics, n)
			}
		}
	}
