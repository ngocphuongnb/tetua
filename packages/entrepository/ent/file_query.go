...
// WithUserAvatars tells the query-builder to eager-load the nodes that are connected to
// the "user_avatars" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FileQuery) WithUserAvatars(opts ...func(*UserQuery)) *FileQuery {
	query := &UserQuery{config: fq.config}
	for _, opt := range opts {
		opt(query)
	}
	fq.withUserAvatars = query
	return fq
}
...
func (fq *FileQuery) sqlAll(ctx context.Context) ([]*File, error) {
	...
	if query := fq.withPosts; query != nil {
		...
		query.Where(predicate.Post(func(s *sql.Selector) {
			s.Where(sql.InValues(file.PostsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := fq.withUserAvatars; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*File)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserAvatars = []*User{}
		}
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(file.UserAvatarsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "avatar_image_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.UserAvatars = append(node.Edges.UserAvatars, n)
		}
	}
